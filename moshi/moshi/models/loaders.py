...
DEFAULT_RAG_REPO = "kyutai/moshika-rag-pytorch-bf16"
...
class CheckpointInfo:
    ...
    def get_moshi(
        ...
        skip_conditioners: list[str] = [],
        **kwargs,
    ) -> LMModel:
        if len(skip_conditioners) > 0:
            for conditioner in skip_conditioners:
                self.lm_config["conditioners"].pop(conditioner)
            for key in self.lm_config["fuser"]:
                fuser = self.lm_config["fuser"][key]
                if isinstance(fuser, list) and conditioner in fuser:
                    self.lm_config["fuser"][key].remove(conditioner)
        model = get_moshi_lm(
...
def get_moshi_lm(
    ...
    if filename is not None:
        if _is_safetensors(filename):
            ...
            model.load_state_dict(pkg["fsdp_best_state"]["model"], assign=True, strict=False)
    if model.condition_provider is not None:
        for _name, cond in model.condition_provider.conditioners.items():
...
def get_conditioner(output_dim: int, device: torch.device | str, conditioner_cfg: dict) -> BaseConditioner:
    ...
    elif conditioner_type == "multi_t5":
        from ..conditioners.text import MultiT5Conditioner

        return MultiT5Conditioner(**conditioner_kwargs)
